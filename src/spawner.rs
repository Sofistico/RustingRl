...
pub fn spawn_room(ecs: &mut World, room: &Rect) {
    ...
    //scope to keep burrow checker happy
    {
        ...
        for _i in 0..num_monsters {
            let mut added = false;
            while !added {
                let idx = get_pos_idx(room, &mut rng) as i32;
    ...
    for idx in item_spawn_points {
        let x = math_util::index_to_x(idx, MAPWIDTH as i32);
        let y = math_util::index_to_y(idx, MAPWIDTH as i32);
        random_items(ecs, x as i32, y as i32);
    }
}

fn get_pos_idx(room: &Rect, rng: &mut specs::shred::FetchMut<RandomNumberGenerator>) -> usize {
...
pub fn random_items(ecs: &mut World, x: i32, y: i32) {
    let roll: i32;
    {
        let mut rng = ecs.write_resource::<RandomNumberGenerator>();
